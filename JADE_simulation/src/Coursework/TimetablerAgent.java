...
public class TimetablerAgent extends Agent {
	...
	public class SynchAgentsBehaviour extends Behaviour {
		...
		private int day = 0;
		ArrayList<AID> preferenceAgent = new ArrayList<>();
		...
		public void action() {
			...
			case 0:
				...
				try{
					...
					for(int i=0; i<agentsType1.length; i++){
						preferenceAgent.add(agentsType1[i].getName()); // this is the AID
					}
					DFAgentDescription[] agentsType2  = DFService.search(myAgent,template2); 
					for(int i=0; i<agentsType2.length; i++){
						preferenceAgent.add(agentsType2[i].getName()); // this is the AID
				...
				tick.setContent("Choose 1 timeslot per ");
				for(AID id : preferenceAgent) {
					tick.addReceiver(id);
				}
				myAgent.send(tick);
				step++;
			...
			case 1:
				...
				if(msg != null) {
					numFinReceived++;
					if(numFinReceived >= preferenceAgent.size()) {
						step++;
					}
				}
				else {
